...
> **A note on what "hints" means here.** Help comes in three forms: a **Tier-2 nudge** (a gentle prompt toast), **RaiseScaffold** (adding visual support / stepping difficulty back), and — on the generated practice stage — a real **hint ladder** ("Need a hint?" → a strategy step, then a concrete first move; never the answer). Using a hint is recorded as `hint_max_rung`, so a hinted correct is **not** hint-free and does not count toward the clean fade streak or the independence gate. The manipulative *teaching* stages remain visual rather than text-hint.
...
1. **Hints are now real (resolved).** The generated practice stage has a hint ladder ("Need a hint?" → strategy → first step). It feeds the engine: a hinted correct is recorded with `hint_max_rung > 0`, so it does **not** count toward the clean fade streak or scaffold-independence — leaning on hints slows the climb and keeps the gate honest. Proven by *test_proof_integration: "leaning on HINTS does not fast-track mastery"* and `test_hints`. (The manipulative teaching stages stay visual rather than text-hint, by design.)
2. **`too_fast_correct` threshold reconciled (resolved).** Both the runtime guard (`_isTooFastCorrect`) and the in-band clean-streak window now use the single source of truth `PARAMS.latencyFloorMs` (1200 ms) — no more 800/1200 split, no `[800,1200)` dead zone where a correct counted toward neither.
