...
| game-ui-design | 3 | Strong readability discipline; goal-communication and per-outcome feedback grammar under-specified. |
...
- **Per-beat animated-node cap + `willChange` hygiene + list/perf for R4 overflow (motion, phaser).** ≤50 animating nodes/beat, `willChange` only on currently-springing nodes. spec-clarifications §9 U7.6.
...
4. *(withdrawn — the non-reader goal-communication spec was dropped with the prerequisite counting/whole-number rooms; section number retained to keep the §-references stable.)*
...
- [ ] **Define "Slice 0" — the thin first vertical (implement-specs).** Cut kitchen opener + R1 only through U1–U8 with explicit acceptance criteria (engine replay green; one socket round-trip; wireframe Skin renders every R1 functional ID+state; the player can identify the next action from on-screen goal communication; Playwright drives kitchen→R1→kitchen by selector). Gate Slice 0 on the schema items above being ratified.
...
- [ ] **Build Phase 1 Slice 0, then fan out room-by-room** (R2, R3, R4), reusing the Slice-0 acceptance template per room.
