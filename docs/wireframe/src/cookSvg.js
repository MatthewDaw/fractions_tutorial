/* cookSvg.js — the Cook tutor figure, shared by the newer lesson modules so the
   big SVG isn't copy-pasted into every screen file. `tutor(ribbon)` returns the
   full inner HTML for a LessonScreen `tutorHTML` slot (Cook + speech ribbon). */
...
export function tutor(ribbon) {
  return `
    <div class="ltutor">${COOK_SVG}
