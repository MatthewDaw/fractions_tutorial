/* kitchen-wrong — RawScreen (one-off scene). Inner-#stage markup captured
...
export default {
  kind: "raw",
  title: "Babushka's Kitchen · Try Again",
  route: "#/mom",
  bodyHTML: `
      ...
      <div class="mr-s">
        ...
        <div class="mr-s-answer">
          <div class="mr-s-write">
            ...
            <!-- WRONG state: bad=true → .mr-s-slate carries \`mr-bad\` (red cells + shakeX).
                 Child wrote 5/16 — the "sameBottom" slip (added tops AND bottoms). -->
            <div class="mr-s-slate mr-bad">
              <span class="frinput">
                <div class="slate slate-fraction" role="group" aria-label="your fraction answer">
                  <!-- top cell (num) — COMMITTED wrong digit "5" (slot.is-committed →
                       canvas removed, committed digit button painted over it) -->
                  ...
                  <!-- bottom cell (den) — COMMITTED wrong digit "16" -->
                  <div class="slate-slot is-committed">
                    <div class="slate-cell committed">
                      <button type="button" class="slate-committed-digit" title="Tap to rewrite">16</button>
                    </div>
                  </div>
                </div>
              </span>
          ...
          <div class="mr-s-marks">
            <!-- No Rosette (not solved). No "Learn it" wall: a single first wrong answer
                 on the mirror stage shows the nudge bubble, not the RouteToRoom wall
                 (the wall fires after the engine detects a confirmed skill gap). -->
            <!-- Check (answer entered but wrong → still reads "Check" → "try again", inert) -->
        ...
        <div class="mr-s-tutor">
          <!-- bubble.who = "mom", mood "think", tone "warn" → Mom expr "think" -->
          <div class="cook-stage">
            <svg width="118" height="156" viewBox="0 0 196 260" style="display:block; overflow:visible" aria-hidden="true">
              ...
              <!-- THINK eyes: pupils + raised brows + a quizzical brow tick -->
          ...
          <!-- ribbon tone "warn" → \`.ribbon.warn\`; text = mr_mom_nudge_samebottom -->
          <div class="ribbon warn" data-vox-speaker="mom">The pieces are the same size, so the bottom number does not grow. Only count the tops.</div>
        </div>
      </div>
    </div>
`,
