...
description: "Use when building or iterating on a web game (HTML/JS) and needs a reliable development + testing loop: implement small changes, run a Playwright-based test script with short input bursts and intentional pauses, inspect screenshots/text, and review console errors with render_game_to_text."
---
...
```bash
export SKILL_DIR="$(git rev-parse --show-toplevel)/.claude/skills/develop-web-game"
...
This skill is installed as a project skill under `.claude/skills/develop-web-game`.
